...
# Merge season-level Elo (use previous year to avoid lookahead)
df_elo['year'] += 1
...
# Get feature importance for margin model
importances_margin = margin_model.feature_importances_
feature_importance_margin = pd.DataFrame({'feature': features, 'importance': importances_margin})
feature_importance_margin = feature_importance_margin.sort_values('importance', ascending=False)
print("Feature Importance for Margin Model:\n", feature_importance_margin)
...
importances_total = total_model.feature_importances_
feature_importance_total = pd.DataFrame({'feature': features, 'importance': importances_total})
feature_importance_total = feature_importance_total.sort_values('importance', ascending=False)
print("Feature Importance for Total Model:\n", feature_importance_total)

# Select top N features (e.g., top 15) based on average importance
average_importance = (feature_importance_margin.set_index('feature') + feature_importance_total.set_index('feature')) / 2
average_importance = average_importance.sort_values('importance', ascending=False)
top_features = average_importance.head(15).index.tolist()

# Retrain models with top features
X_top = df_train[top_features]
X_train_top, X_test_top, y_margin_train, y_margin_test = train_test_split(X_top, y_margin, test_size=0.2, random_state=42)
margin_model_top = XGBRegressor(n_estimators=100, learning_rate=0.1, random_state=42)
margin_model_top.fit(X_train_top, y_margin_train)
print(f"Margin MAE with top features: {mean_absolute_error(y_margin_test, margin_model_top.predict(X_test_top))}")

X_train_top, X_test_top, y_total_train, y_total_test = train_test_split(X_top, y_total, test_size=0.2, random_state=42)
total_model_top = XGBRegressor(n_estimators=100, learning_rate=0.1, random_state=42)
total_model_top.fit(X_train_top, y_total_train)
print(f"Total MAE with top features: {mean_absolute_error(y_total_test, total_model_top.predict(X_test_top))}")

# Use top models for predictions
margin_model = margin_model_top
total_model = total_model_top
...
# Engineer rolling Elo for current from per-game pre-game Elo in current_all_games
# Fetch all 2025 games for Elo history
